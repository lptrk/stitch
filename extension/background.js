...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "VALIDATE_SELECTOR") {
    const sendToTab = (targetId) => {
      if (!targetId) { sendResponse({ count: 0, error: "No app tab found – pick an element once, or open your app in a tab first" }); return }
    ...
    if (msg.tabId || pickingTabId) {
      sendToTab(msg.tabId || pickingTabId)
    } else if (msg.baseUrl) {
      // No pick has happened yet this session – resolve the app tab the same way START_PICKING does.
      chrome.tabs.query({ currentWindow: true }, (allTabs) => {
        getStitchUrl((stitchUrl) => {
          let target = null
          try {
            const base = new URL(msg.baseUrl).origin
            target = allTabs.find(t => {
              try { return new URL(t.url).origin === base && !isStitchTab(t.url, stitchUrl) } catch { return false }
            })
          } catch {}
          sendToTab(target?.id)
        })
